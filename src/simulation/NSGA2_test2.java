...
public class NSGA2_test2 {
	...
	*/
	...
	Comparator<Individual> comparator_crowing2 = new Comparator<Individual>() {
		@Override
		public int compare(Individual obj1, Individual obj2) {
			int result = Double.valueOf(obj2.crowing).compareTo(Double.valueOf(obj1.crowing));
			return -result;
	...
	//コンストラクタ
	NSGA2_test2(ArrayList<Node> list1, ArrayList<Link> list2, double r_share,long limit) {
		this.limit = limit;
		this.nodelist = list1;
		this.linklist = list2;
...
//				(pareto_group.get(pareto_group.size() - 1)).crowing = 10000;
				...
				for (int i = 1; i < pareto_group.size() - 1; i++) {
					value=(Math.abs((pareto_group.get(i - 1)).eva_distance - (pareto_group.get(i + 1)).eva_distance))/ normalize_dist;
...
//				Collections.sort(pareto_group, comparator_crowing2);
